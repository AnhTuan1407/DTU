...
export class CategoryComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.categoryFormGroup = new FormGroup({
      categoryForm: new FormControl('', [Validators.required, Validators.minLength(6), Validators.maxLength(20)])
    })
  }
  ...
  createCategory(){
    console.log("categoryname" + this.categoryFormGroup.value.categoryForm);
    const {name} = this.categoryForm;
    this.categoryService.createCategory(this.categoryFormGroup.value.categoryForm).subscribe({
      next: res =>{
        this.getListCategory();
